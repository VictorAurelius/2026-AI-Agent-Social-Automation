...
```
Credential Name: Social Automation DB
Host: postgres
Port: 5432
...
- PostgreSQL nodes → phải link tới "Social Automation DB" credential
...
2. Get bot token và chat ID (xem `docker/telegram-config.md`)
3. Update `docker/.env`:
   ```
